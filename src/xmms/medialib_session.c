...
static GHashTable *xmms_medialib_session_get_table (GHashTable **table);
                                      ...
                                      s4_transaction_flag_t flags)
{
	...
	ret->sourcepref = xmms_medialib_get_source_preferences (medialib);
...
{
	return s4_sourcepref_ref (session->sourcepref);
}
...
{
	s4_sourcepref_unref (session->sourcepref);
...
static GHashTable *
xmms_medialib_session_get_table (GHashTable **table)
{
	if (*table == NULL)
